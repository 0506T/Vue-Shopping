  ...
  <div class="cart">
    ...
    <div v-if="isLogin && cartList.length > 0">
      ...
      <div class="cart-item" v-for="item in 10" :key="item">
        ...
        <div class="info">
          ...
          <span class="bottom">
            ...
            <ContBox @input="(value) => changeCount(value, item.goods_id, item.goods_sku_id)"></ContBox>
          </span>
        </div>
      </div>
      </div>
      ...
      <div class="all-total">
        ...
        <div v-if="true" class="goPay" @click="goPay">结算(5)</div>
        <div @click="handleDel" v-else class="delete">删除</div>
      </div>
      </div>
    </div>
...
export default {
  ...
  computed: {
    ...
    ...mapGetters('cart', ['cartTotal', 'selCartList', 'selCount', 'selPrice', 'toggleAllCheck']),
    isLogin () {
  ...
  methods: {
    ...
    toggleAllCheck () {
      this.$store.commit('cart/toggleAllCheck', !this.toggleAllCheck)
    },
